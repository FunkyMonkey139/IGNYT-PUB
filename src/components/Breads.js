...
function Breads() {
  const [showBread, setBreads] = useState([]);
  let cart;
  let count = 0;
    ...
    const getBread = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8008/api/displayDish/showBread"
        );
        setBreads(response.data.breads);
      } catch (error) {
        console.error("Error fetching bread details:", error);
      }
    };
    getBread();
  }, []);
