...
function Barbites() {
  ...
  const [showBarbites, setBarbites] = useState([]);
  let cart;
  let count = 0;
  ...
  useEffect(() => {
    const getBarbites = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8008/api/displayDish/showBarbite"
        );
        setBarbites(response.data.barbites);
      } catch (error) {
        console.error("Error fetching barbites details:", error);
      }
    };
    getBarbites();
  }, []);
  ...
  return (
    <div className="food-container">
      ...
      <table className="food-table">
        ...
        <tbody>
          {showBarbites.map((service, index) => (
            <tr key={index}>
