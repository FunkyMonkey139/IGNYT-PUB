...
function Soups() {
  const [showSoup, setSoup] = useState([]);
  let cart;
  let count = 0;
    ...
    const getSoups = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8008/api/displayDish/showSoup"
        );
        setSoup(response.data.soups);
      } catch (error) {
        console.error("Error fetching soup details:", error);
      }
    };
    getSoups();
  }, []);
