...
function Milkshake() {
  const [showMilkshake, setMilkshake] = useState([]);
  let cart;
  let count = 0;
  ...
  useEffect(() => {
    const getMilkshakes = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8008/api/displayDish/showMilkshake"
        );
        setMilkshake(response.data.milkshakes);
      } catch (error) {
        console.error("Error fetching milkshake details:", error);
      }
    };
    getMilkshakes();
  }, []);
  ...
  return (
    <div className="food-container">
      ...
      <table className="food-table">
        ...
        <tbody>
          {showMilkshake.map((service, index) => (
            <tr key={index}>
