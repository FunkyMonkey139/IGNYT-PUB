...
function Platters() {
  const [showPlatter, setPlatters] = useState([]);
  let cart;
  let count = 0;
  ...
  useEffect(() => {
    const getPlatters = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8008/api/displayDish/showPlatter"
        );
        setPlatters(response.data.platters);
      } catch (error) {
        console.error("Error fetching platters details:", error);
      }
    };
    getPlatters();
  }, []);
