...
function Tandoori() {
  const [showTandoori, setTandoori] = useState([]);
  let cart;
  let count = 0;
  ...
  useEffect(() => {
    const getTandoori = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8008/api/displayDish/showTandoori"
        );
        setTandoori(response.data.tandoori);
      } catch (error) {
        console.error("Error fetching tandoori details:", error);
      }
    };
    getTandoori();
  }, []);
  ...
  return (
    <div className="food-container">
      <h1><i>Straight Outta Tandoori</i></h1>
      <table className="food-table">
        ...
        <tbody>
          {showTandoori.map((service, index) => (
            <tr key={index}>
